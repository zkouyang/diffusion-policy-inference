...
from diffusion_policy.model.ACT.transformer import (
    Transformer,
    TransformerEncoder,
...
class ActionChunkTransformerPolicy(BaseImagePolicy):
    ...
    def compute_loss(self, batch, teacher_policy=None):
        ...
        # generate DDPM model_out as tareget
        if teacher_policy is not None:
            with torch.no_grad():
                teacher_result = teacher_policy.predict_action(nobs)
                tgt_nactions = teacher_result["naction_pred"]
        else:
            tgt_nactions = nactions
        ...
        loss_dict = dict()
        all_l1 = F.l1_loss(tgt_nactions, a_hat, reduction="none")
