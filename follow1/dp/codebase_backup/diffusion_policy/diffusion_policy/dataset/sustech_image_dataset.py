...
class SustechImageDataset(BaseImageDataset):
    def __init__(
        self,
        dataset_dir: str,
        shape_meta: dict,
    ...
    def load_data(
        ...
        for i in tqdm(range(num_episodes)):  # num_episodes
            ...
            with h5py.File(dataset_path, "r") as root:
                qpos = root["/observation/qstate"][()]
                qvel = root["/observation/qdstate"][()]
                action = root["/action"][()]

                # stack different cameras
                all_cam_images = []
                for cam_name in camera_names:
                    all_cam_images.append(root[f"/observation/images/{cam_name}"][()])
                all_cam_images = np.stack(all_cam_images, axis=0) # [n_cam, T, H, W, C]

            episode = {
                "qpos": qpos, # [T, dim]
                "action": action, # [T, dim]
                "images": all_cam_images.squeeze(0), # [T, H, W, C]
            }
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=n_workers) as executor:
            ...
            for key in rgb_keys:
                ...
                for i in range(num_episodes):
                    dataset_path = os.path.join(dataset_dir, f"episode_{i}.hdf5")
                    with h5py.File(dataset_path, "r") as demo:
                        data_key = "observation/images/" + key
